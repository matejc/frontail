...
window.App = (function (window, document) {
     ...
     * @private
     */
    var _highlightRegExp = function (line) {
        console.log(_highlightConfig);
        if (_highlightConfig) {
            if (_highlightConfig.replace) {
                for (var regexp in _highlightConfig.replace) {
                    if (_highlightConfig.replace.hasOwnProperty(regexp)) {
                        console.log(regexp, ", ", line);
                        line = line.replace(
                            new RegExp(regexp, _highlightConfig.replace[regexp].flags),
                            _highlightConfig.replace[regexp].text
                        );
                        console.log(line);
                    }
                }
    ...
    return {
        ...
        log: function (data) {
            ...
            // Escape HTML tags
            data = _escape(data);
            var text = _highlightWord(data);
            p.innerHTML = _highlightRegExp(text);
